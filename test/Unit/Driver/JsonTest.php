<?php

declare(strict_types=1);

/**
 * Copyright 2013-2026 The Horde Project (http://www.horde.org/)
 *
 * See the enclosed file LICENSE for license information (LGPL). If you
 * did not receive this file, see http://www.horde.org/licenses/lgpl21.
 *
 * @author   Michael Slusarz <slusarz@horde.org>
 * @license  http://www.horde.org/licenses/lgpl21 LGPL 2.1
 */

namespace Horde\Pack\Test\Unit\Driver;

use Horde_Pack_Driver_Json;
use Horde_Pack_Exception;
use PHPUnit\Framework\Attributes\CoversClass;

#[CoversClass(Horde_Pack_Driver_Json::class)]
class JsonTest extends TestBase
{
    protected string $drivername = 'Horde_Pack_Driver_Json';

    public function testInvalidUtf8(): void
    {
        $this->expectException(Horde_Pack_Exception::class);
        $this->expectExceptionMessage('Could not pack data.');
        self::$pack->pack(chr(228), [
            'drivers' => ['Horde_Pack_Driver_Json'],
        ]);
    }
}
